...
EXPLICIT_UNTRUSTWORTHY_SITES = [
    "quora", "reddit", "twitter", "forum", "u/", "r/"]
...
with open('./data/text-classification/wikitext-103/wiki.train.tokens', 'r') as f:
    ...
    WIKIPEDIA_PASSAGES = WIKIPEDIA_PASSAGES[:(
        len(EXPLICIT_UNTRUSTWORTHY_SITES_PASSAGES)+len(ADVERTISEMENT_PASSAGES))]
...
for qp in EXPLICIT_UNTRUSTWORTHY_SITES_PASSAGES:
    features.append(
        f"This is an unreliable passage from an untrustworthy website: {qp}")
